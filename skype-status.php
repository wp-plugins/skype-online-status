...
/*
	...
	Revision History
		[2007-  -  ] version 2.6: 
			...
			- TODO: classify skype function, chop up single file
			- Todo: use snoopy.class for version checking if possible
...
include(ABSPATH . 'wp-content/plugins/skype-online-status/buttonsnap.php');
	
// --- settings ---

// PID number to use with the Donwload Skype now! link
define('SKYPEPID', '266509');

// Plugin version number. Don't edit unless you are sure.
define('SOSVERSION', '2.5');
define('SOSVERSION_DATE', '2007-02-18');
...
// load databse options
$skype_status_config = get_option("skype_status");

// Add buttons in WordPress v2.1+, thanks to An-archos
function sos_mce_plugins($plugins) {
	array_push($plugins, '-sosquicktag');
	return $plugins;
}
function sos_mce_buttons($button) {
	//array_push($button, 'separator');
	array_push($button, 'sosquicktag');
	return $button;
}
function sos_tinymce_before_init() {
	echo "tinyMCE.loadPlugin('sosquicktag', '" . get_bloginfo('wpurl') . '/wp-content/plugins/skype-online-status/' . "tinymce/');\n"; 
}
...
function skype_button_init() {
	...
	if ( 3664 <= $wp_db_version && 'true' == get_user_option('rich_editing') ) {
		...
		add_filter('mce_plugins', 'sos_mce_plugins');
		add_filter('mce_buttons', 'sos_mce_buttons');
		//add_filter('mce_buttons_0', 'sos_mce_buttons');
		...
		*/
		...
		$url = get_settings('siteurl') . '/wp-content/plugins/skype-online-status/skype_button.gif';
...
function skype_button_css() {
	$skype_marker_url = get_settings('siteurl') . '/wp-content/plugins/skype-online-status/skype_marker.gif';
	echo "
		.skype_marker {
			display: block;
			height: 15px;
...
if ($skype_status_config['skype_status_version'] !== SOSVERSION) {
	...
	// update: populate db with missing values and set upgrade flag to true
...
function skype_status_check($skypeid, $format=".txt") {
	$str = "error";
	if (ALLOWURLFOPEN) { 
		if (function_exists('file_get_contents')) $tmp = file_get_contents('http://mystatus.skype.com/'.$skypeid.$format);
...
function skype_parse_theme($config) {
	...
	if ($config['use_voicemail']!="on") {
		...
		$config['button_template'] = str_replace("<!-- voicemail_start -->","",$config['button_template']);
		$config['button_template'] = str_replace("<!-- voicemail_end -->","",$config['button_template']);	
	}
	
	// after that, delete first line <!-- (.*) -->
	$config['button_template'] = preg_replace("|<!-- (.*) - http://www.skype.com/go/skypebuttons -->|","",$config['button_template']);
...
function skype_status_options() {
	...
	if (ALLOWURLFOPEN) { 
		if (function_exists('file_get_contents')) $wp_plugins_net = unserialize( file_get_contents('http://wp-plugins.net/get_plugin_data.php?id=1147') );
		else $wp_plugins_net = unserialize( implode('', file('http://wp-plugins.net/get_plugin_data.php?id=1147')) );
	}
	...
	if ($wp_plugins_net && SOSVERSION < $latest_version) { 
		echo "<div class=\"updated fade-ff0000\"><p><strong>There is an upgrade available!</p><p>You have Skype Online Status version ".SOSVERSION.". Please check <a href=\"http://www.wp-plugins.net/plugin/skype-status/#plugin_1147\">http://www.wp-plugins.net/plugin/skype-status/#plugin_1147</a> for version $latest_version ($latest_date).</strong></p><p></p></div>";
	}
		...
		<fieldset class="options"><legend>Display options</legend>
			...
			<ul style="list-style: square;">
				...
				<li><label for="use_status">Use <strong>Status text</strong> for the {status} tag?*</label> <select name="use_status" id="use_status">
						...
						<option value="en"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "en" ) print " selected=\"selected\""; ?>>Skype default in English</option>
						<option value="fr"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "fr" ) print " selected=\"selected\""; ?>>Skype default in French</option>
						<option value="de"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "de" ) print " selected=\"selected\""; ?>>Skype default in German</option>
						<option value="ja"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "ja" ) print " selected=\"selected\""; ?>>Skype default in Japanese</option>
						<option value="zh-cn"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "zh-cn" ) print " selected=\"selected\""; ?>>Skype default in Chinese</option>
						<option value="zh-tw"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "zh-tw" ) print " selected=\"selected\""; ?>>Skype default in Taiwanese</option>
						<option value="pt"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "pt" ) print " selected=\"selected\""; ?>>Skype default in Portuguese</option>
						<option value="pt-br"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "pt-br" ) print " selected=\"selected\""; ?>>Skype default in Brazilian</option>
						<option value="it"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "it" ) print " selected=\"selected\""; ?>>Skype default in Italian</option>
						<option value="es"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "es" ) print " selected=\"selected\""; ?>>Skype default in Spanish</option>
						<option value="pl"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "pl" ) print " selected=\"selected\""; ?>>Skype default in Polish</option>
						<option value="se"<?php if (!ALLOWURLFOPEN) echo " disabled=\"disabled\""; elseif ( $option['use_status'] == "se" ) print " selected=\"selected\""; ?>>Skype default in Swedish</option>
					</select><br />
	...
	<div id="notes" class="wrap" style="min-height: 800px;">
		...
		<p>This is <strong>version <?php echo SOSVERSION; ?></strong> of the Skype Online Status plugin for WordPress 2+.<br />
			...
			The latest release is: <strong>version <?php echo $latest_version; ?></strong> (date <?php echo $latest_date; ?>) on <a href="http://www.wp-plugins.net/plugin/skype-status/#plugin_1147">Skype Online Status on WP-Plugins.net</a>.</p>
...
function skype_status_callback($content) {
	...
	// run the input check
	if(strstr($content,'<!--skype status-->')) {
